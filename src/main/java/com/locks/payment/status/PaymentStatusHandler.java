...
/**
 * Servlet implementation class PaymentStatusHandler
 */

@WebServlet("/PaymentStatusHandler")
public class PaymentStatusHandler extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// System.out.println("inside posttttttt");
		...
		if(status.equalsIgnoreCase("paymentSuccess")) {
			respObj.put("responseCode", "LOCKOPEN-200");
		} else if(status.equalsIgnoreCase("payFailCancel")) {
			respObj.put("responseCode", "LOCKOPENCA-200");
		} else if(status.equalsIgnoreCase("payFailPaylater")) {
			respObj.put("responseCode", "LOCKOPENPL-200");
		}
